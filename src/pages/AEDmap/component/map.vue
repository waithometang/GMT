...
export default {
  data () {
    return {
      ...
      WaChatConfigData: "",
      state: 0,
      icon: require("../images/AED.png"),
      iconAfter: require("../images/AED-blue.png")
    }
  },
  props: ["aedList", "currentName"],
  mounted () {
    // 微信接口获取所在地经纬度
    this.getLocation()
    setTimeout(() => {
  ...
  methods: {
    ...
    init () {
      ...
      // 获取AED点位数据
      var that = this
      this.$post("api/mlgm/gmaedlist").then(res => {
        if (res.data.success) {
          res.data.content.forEach(element => {
            if (element.lb === this.currentName) {
              this.markerArr.push({
                id: element.id,
                styleId: "marker",
    ...
    renderList (lat, lng, geometries = this.marker.geometries) {
      ...
      that.$emit("update:aedList", that.listData.slice(0, 10))
    },
    sortData (a, b) {
      return a.jl - b.jl
    },
    ...
    switchPoints () {
      ...
      if (this.currentName === "急救点") {
        this.icon = require("../images/medical.png")
        this.iconAfter = require("../images/medicalAfter.png")
      } else {
        this.icon = require("../images/AED.png")
        this.iconAfter = require("../images/AED-blue.png")
      }
      this.map.easeTo(
        {
          zoom: 12
      ...
      this.$post("api/mlgm/gmaedlist").then(res => {
        if (res.data.success) {
          ...
          this.marker.setGeometries(this.markerArr)
          this.setStyle(this.marker, 30, 35, this.icon)
        } else {
          this.changtoast(res.data.message, "error")
        }
      })
    ...
    search (query) {
      ...
      this.$post('api/mlgm/gmaedSealist', { Search: query, lb: lb }).then(res => {
