<template>
  <div class="index">
    <div class="head">
      <div class="icon"><img src="/static/zkbm/zhbanner.png" alt=""></div>
      <div class="category">
    ...
    <div class="procedure">
      ...
      <div class="procedure-list">
        <div class="bar" v-for="(item, index) in listData" :key="index">
          ...
          <div class="procedure-bar"><div class="bar-icon"><img src="/static/ybmimages/jiantou_2.png" alt=""></div></div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
      ...
      this.$get('api/zkgl/ckeckcc').then(res => {
        ...
        if(res.data.success){
          ...
          if(cont===-1){
            this.$dialog.alert({
                  title: "温馨提示",
                  message: "抱歉,当前没有可以报考的场次"
                })
          }else if (cont===-2) {
            this.$router.push('/zkbm/wybm')
          }else{
            this.$router.push('/zkbm/xzgw/' + cont)
          }
        }else{
    ...
    wycxFn() {
      this.$router.push('zkbm/wdbm/list')
    },
    lookOver (item) {
      if (item.linkage) {
