<template>
    ...
    <van-skeleton title :row="10" :loading="!xsData">
      <template v-if="xsData">
        <div class="module-tag" v-if="xsData.zls && xsData.zls.length">
          <div class="module-title">证明材料:</div>
          <div class="list-tab" v-for="(item, index) in xsData.zls" :key="index">
            <div class="publicUploadPictures">
              ...
              <div class="state" v-if="item.sczt==='已审查'">
                结果:
                <span :class="{'color1': item.scjg === '通过'}">{{item.scjg}}</span>
                &nbsp;&nbsp;&nbsp;&nbsp; 审核时间:{{item.scsj}}
              </div>
...
export default {
  ...
  created() {
    ...
    this.$get('api/zkgl/getzmcl?id='+this.$route.query.bmid).then(rps => {
