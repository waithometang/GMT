    ...
    <div class="jtcy">
      ...
      <van-form @submit="onSubmit" ref="form" label-align="right" :scroll-to-error="true">
        <div class="cyxx" v-for="(item,index) in jtcys" :key="index">
          ...
          <datetimeSelector :minDate="birthminDate" v-model="item.csrq" label="出生年月" :readonly="isView" />
          <GmtField v-model="item.xgzdw" label="现工作单位" :readonly="isView"/>
          <GmtField v-model="item.xgzzw" label="现工作职务" :readonly="isView"/>
          <i v-if="!isView" class="van-icon van-icon-clear van-uploader__preview-delete" @click="()=>delFn(index)"></i>
        </div>
      </van-form>
      <div class="base btn">
        <div class="base-tab width">
          <div class="public-long-but" @click="addMember" v-if="!isView">新增成员</div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      this.$store.state.zkbmData.jtcys=this.jtcys
      localStorage.setItem('tempZkbmDataJTCY',JSON.stringify(this.jtcys))
  ...
  created () {
    if (this.bmid) {
      ...
      this.$get('api/zkgl/jtcylist?id='+this.bmid).then(res => {
      ...
      let jtcysStr=localStorage.getItem('tempZkbmDataJTCY')
      if (jtcysStr) {
