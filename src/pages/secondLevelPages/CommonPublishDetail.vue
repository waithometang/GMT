...
import { GetPulishDetail, GetMark, GetFavorite, GetUndoMark, GetUndoFavorite, Getfbpl, GetPlList, GetPlDel } from "@/api/secondLevelPage.js";
    ...
    _GetDetailPages() {
      GetPulishDetail({
        ...
        if (rps.data.success) {
          ...
          setTimeout(() => {
            this.$nextTick(() => {
              let dom=document.getElementById('listDataContent')
              if (dom) {
                let content = dom.innerText.replace(/\s/g,'')
                // let url = this.listData.cover
                let url = _data.cover
                WaChatrequireShare(this.listData.title, content, `nrfbxx/${this.mkid}/${this.id}` , url , this)
              }
            })
          }, 1500);
        } else {
