<template>
  <div class="gwxx">
    <van-row class="header">
      ...
      <van-col span="10" :offset="2" align="right" class="tj"
        >合计职位：{{ listdata.length }}个</van-col
      >
    </van-row>
    <div class="gwxx-list">
      ...
      >
        <div class="gwxx-item" :class="{ active: selected.id === item.id }">
          ...
          <div class="rs">招聘人数：{{ item.zkrs }}</div>
          <div class="bzsm">说明：<span v-html="item.bz"></span></div>
        </div>
      </div>
    </div>
    <div class="base-tab">
    ...
    confirmFn() {
      ...
      this.$post("api/zkgl/CheckCcxx", {
        ...
        if (res.data.success) {
          ...
          this.$router.push("/dwzk/txxx" + window.location.search)
        } else {
          this.changtoast(res.data.message, "error")
        }
      })
