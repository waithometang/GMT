...
export default {
  ...
  methods: {
    ...
    viewDetails (item) {
      this.$router.push("/dwzk/jbxx?m=v&bmid=" + item.id)
    },
    closeFn (action, done) {
      if (action === "confirm") {
        if (!this.zjhm) {
        ...
        this.$post("ybm/xxapi/IsInPBMD", { name: this.zjhm }).then(res => {
          ...
          if (res.data.success) {
            if (res.data.content) {
              ...
              this.$router.push({
                name: "bmbzxxtxxx",
                params: { zjhm: this.zjhm }
              })
            } else
              this.errorMes = "您未提交补报名申请，如有疑问请致电88282386咨询"
          } else {
