<template>
  <div class="releaseFrom" v-if="showPages">
    <div class="content">
      <div class="input_tab" v-if="this.lx === 3">
    ...
    <div class="gradeList" v-if="this.lx === 3 && khsx.length > 0">
      ...
      <div class="file_table">
        <table class="f-table" cellspacing="0" cellpadding="0" width="100%">
          ...
          <tbody>
            <tr v-for="(items, index) in khsx" :key="index">
              <td>
                <span class="font">
                  {{index+1}}: {{items.khnr}}
                  <span v-if="items.isNeed" class="startMark">（*）</span>
                </span>
              </td>
              <td>{{items.sfz}}</td>
              <td>
                <uploadFile :title="items.khnr" :dataObject="items" @uploadFinished="uploadFinished" :ref="'zl_'+items.id" :WaChatConfigData="WaChatConfigData" :files="items.files" :mainIdData="items.mainId" @removeImage="remove"/>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <div class="base">
      <div class="release_but" @click="_DelGrfb()">
        删除发布
      </div>
    </div>
...
import { GetGrfb, GetWdfb, Grfbxx, EditGrfb, DelGrfb, Getzjxx } from "@/api/secondLevelPage"
  ...
  computed: {
    ...
    uploadFinished(fileCount,item){
      ...
      if (item.fz && item.kfbl) {
        ...
        if (rtn>0) {
          item.sfz=rtn
          this.totalPoints = this.totalPoints - item.kfbl*fileCount
        }else{
          item.sfz=0
        }
        this.settotalPoints()
    ...
    settotalPoints () {
      this.totalPoints = 0
      for (let index = 0; index < this.khsx.length; index++) {
        const element = this.khsx[index];
        if (element.sfz) {
    ...
    GetGrfbxx () {
      ...
      Grfbxx({
        ...
        if (rps.data.success) {
          ...
          this.lx = rps.data.content.lx - 1
          if (rps.data.content.sszj) {
            this.zjInfo = rps.data.content.sszj.zj // 镇街信息
            this.cjInfo = rps.data.content.sszj.cj // 村居信息
            this.zrcInfo = rps.data.content.sszj.zrc //自然村信息
          }
          rps.data.content.khsx.forEach(item => {
            item.sfz=item.fz*1 - item.files.length * item.kfbl
          })
          this.khsx = rps.data.content.khsx
          for (let index = 0; index < this.khsx.length; index++) {
            const element = this.khsx[index]
            if (element.sfz) {
              this.totalPoints = this.totalPoints + element.sfz*1
    ...
    onValuesChange (picker, values) {
      ...
      // if (this.tabState === 1) {
      //   this.zjInfo = values[0]
      // } else if (this.tabState === 2) {
      //   this.cjInfo = values[0]
      // } else if (this.tabState === 3) {
      //   this.zrcInfo = values[0]
      // }
    },
    // 获取考核事项文件id
    GetKhsxId () {
