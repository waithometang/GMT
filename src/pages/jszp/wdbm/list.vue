    ...
    <div class="middle">
      ...
      <div class="middle-tab" v-for="(item, index) in list" :key="index" @click="viewDetails(item)">
        ...
        <div class="tab-right">
          ...
          <div class="right-tab">报名类型:<span>{{item.bmlx}}</span></div>
          <div class="right-tab">报考单位:<span>{{item.bkdw}}</span></div>
          <div class="right-tab">报考职位:<span>{{item.gwmc}}</span></div>
          ...
          <div class="right-base">
            ...
            <div class="base-right" v-if="item.sczt==='待审查'">资格审查中></div>
...
export default {
  ...
  methods: {
    ...
    viewDetails(item){
      if(item.scjg == '通过'){
        this.$router.push('/jszp/jdjg?m=v&bmid='+item.id)
      }else{
        this.$router.push('/jszp/jbxx?m=v&bmid='+item.id)
      }
    },
    closeFn(action,done) {
      if (action==='confirm') {
    ...
    getListData () {
      ...
      this.$post('api/jszp/wdbm',_params).then(rps=>{
