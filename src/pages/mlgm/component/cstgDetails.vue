    ...
    <div class="container">
      <div class="swiperList">
        ...
        <div class="wrap">
          <div class="tab_box">
            <div class="tag">
              <div class="label" v-for="item in getArr(labels)" :key="item">{{item}}</div>
            </div>
          </div>
        ...
        <!-- v-if="form.shzt == '已通过' && !form.hasJF" -->
        <!-- v-if="form.mine && form.fbzt=='已发布' && !form.hasJF" -->
        <div class="warn" v-if="form.mine && form.fbzt=='已发布' && !form.hasJF">
          <img src="../images/tips.png" alt />
          <span class="tip">您还没上传加分项哦！请点击下方按钮上传加分项</span>
        </div>
      </div>
      <!-- 已审核并且是选择的奖品时候显示核销码 -->
      <div class="verCode" v-if="form.fbzt=='已发布' && form.mine" @click="showHxmFn">
        <img src="../images/tips.png" alt class="tips" />
        恭喜您本作品获得小奖品一份，点击奖品核销码兑奖吧→
        <img src="../images/vercode.png" alt class="code" />
      </div>
      <div class="contentwrap">
        <div class="content" v-html="handlerHh(form.zpms)"></div>
         <div v-if="form.videoSrc" class="pic_detail">
          <div class="pic_list" v-for="(image, index) in form.pics" :key="index" @click="() => preview(form.pics,index)">
            <img :src="image" alt="">
          </div>
      </div>
      </div>
      <!-- 未审核 -->
      <div class="group_btn" v-if="form.shzt!='已通过'">
        <!-- <div class="grade_btn" @click="addGrade">
      ...
      <!-- 已审核 -->
      <div class="group_ckbtn" v-else>
        <!-- 未上传加分项 -->
        <div v-if="!form.hasJF" class="notUp">
          <div class="box">
            <div class="up_grade" @click="addGrade" v-if="form.mine && form.fbzt=='已发布'">
              <img src="../images/up.png" alt />
              上传加分项
            </div>
        ...
        <div class="hasUp" v-else>
          <div class="grade_btn" @click="showTips" :class="{'call_btn':btnFlag}">
            <img src="../images/thumb_1.png" alt v-if="!btnFlag"/>
            <img src="../images/good.png" alt="" v-else>
            {{!btnFlag ? '为他打call' : '已打call'}}
          </div>
...
export default {
  ...
  methods: {
    showHxmFn() {
      this.$router.push("/mlgm/cstghxm?id=" + this.$route.params.id);
    },
    onChange(index) {
      this.current = index;
    },
